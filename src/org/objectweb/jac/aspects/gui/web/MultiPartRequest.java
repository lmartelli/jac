 ...
 * </pre>
 *
 * @version $Id: MultiPartRequest.java,v 1.6 2004/03/29 12:52:09 laurent Exp $
 * @author  Greg Wilkins
 * @author  Jim Crossley
 */
public class MultiPartRequest
{
    ...
    String _enc = StringUtil.__ISO_8859_1;

    /* ------------------------------------------------------------ */
    /** Constructor. 
     * @param request The request containing a multipart/form-data
     * request
     * @param enc the encoding of characters
     * @exception IOException IOException
     */
    public MultiPartRequest(HttpServletRequest request, String enc)
        throws IOException
    {
        _request=request;
        _enc = enc;
    ...
    protected Part getPart(String name) {
       Object part = _partMap.get(name);
       if (part instanceof List)
          return (Part)((List)part).get(0);
       else
          return (Part)part;
    }
    ...
    {
        return (String[]) _partMap.keySet().toArray(ExtArrays.emptyStringArray);
    }
    ...
    public String getString(String name)
    {
        ...
        if (part==null)
            return null;
        return decodeString(((Part)part.get(0))._data);
    }
    ...
    public String[] getStrings(String name)
    {
        ...
        for (int i=0; i<strings.length; i++) {
            strings[i] = decodeString(((Part)parts.get(i))._data);
        }
        return strings;
    }
    ...
    protected String decodeString(byte[] bytes) {
        try {
            return new String(bytes,_enc);
        } catch (UnsupportedEncodingException e) {
            logger.error("Unsupported encoding: "+_enc);
